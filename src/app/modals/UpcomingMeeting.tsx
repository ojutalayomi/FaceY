						...
						<div className='flex min-h-full items-center justify-center p-4 text-center'>
							...
							>
								<DialogPanel className='w-full max-w-2xl transform overflow-hidden rounded-2xl bg-gradient-to-br from-gray-900 to-gray-800 text-white p-6 align-middle shadow-xl transition-all text-center'>
									<DialogTitle
										as='h3'
										className='text-lg font-bold leading-6 text-brand mb-4'
									>
										Upcoming Call
									</DialogTitle>
			...
			<div className='flex flex-col space-y-4'>
				{upcomingCalls?.map((call: Call) => (
                    ...
                    <div className="w-2/3 flex items-center space-x-4 justify-between">
							...
							<p className='text-xs'>{formatDateTime(String(call.state?.startsAt?.toLocaleString()))}</p>
                    </div>
                    
						<Link className='bg-brand/80 text-sm px-4 py-2 hover:bg-brand text-white rounded-md shadow-sm'
						href={`/Call/${call.id}`}
						>
                       Start now
                    </Link>
